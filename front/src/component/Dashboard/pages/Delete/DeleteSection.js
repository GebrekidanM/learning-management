...
function DeleteSection({name,onDeleteSuccess,setDeleteCard,id}) {
  ...
  return (
    <div 
        ...
        <Card className='w-4 bg-cyan-900 text-white text-center'>
            <h2>Grade {name}</h2>
            <p>Do you want to <span className='text-yellow-500'>Delete</span> this grade?</p>
            <div className='w-8 mx-auto flex justify-content-between'>
              <button className={`button ${isDisabled ? 'bg-cyan-700 cursor-wait' : 'bg-yellow-700'} hover:bg-cyan-700`} disabled={isDisabled} onClick={handleDelete}>Delete</button>
